...
2. Copy .env.example to .env then configure it
...
2. Service got a webhook
   1. If status created or running
      1. Check if we need send start event to virtual machine by:
         ...
         2. If it's new job (send because we can, maybe it doesn't need but bugs or etc.)
         3. Previous start time < current date - $RESEND_START_INSTANCE_AFTER
   2. If status success
      1. Check if all jobs finished 
...
4. Select `Job events` in `Trigger`
